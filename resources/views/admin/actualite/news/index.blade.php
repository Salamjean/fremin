...
<style>
    ...
    .article-card:hover .article-order {
            ...
            @else
                <div class="articles-grid" id="articlesGrid">
                    ...
                    <div class="article-card {{ !$article->is_active ? 'inactive' : '' }} {{ $article->is_event ? 'event' : '' }}" 
                         ...
                         data-is-event="{{ $article->is_event ? 'true' : 'false' }}"
                         data-date="{{ $article->date }}"
                         data-event-date="{{ $article->event_date }}"
                        ...
                        <div class="article-content">
                            ...
                            <div class="article-stats">
                                <div class="stat-item">
                                    ...
                                    <span>Vues: <span class="stat-value">{{ number_format($article->views, 0, ',', ' ') }}</span></span>
                                </div>
                                @if($article->is_event && $article->event_registrations)
                                    <div class="stat-item">
                                        <i class="fas fa-users"></i>
                                        <span>Inscriptions: <span class="stat-value">{{ number_format($article->event_registrations, 0, ',', ' ') }}</span></span>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="article-meta-footer">
                                ...
                                <div class="article-actions">
                                    ...
                                    <!-- Statut événement -->
                                    <form action="{{ route('admin.news.toggle-event-status', $article) }}" 
                                          method="POST" 
                                          class="d-inline event-form">
                                        @csrf
                                        <button type="button" 
                                                class="btn-action btn-event toggle-event-btn"
                                                data-title="{{ $article->title }}"
                                                data-is-event="{{ $article->is_event }}"
                                    ...
                                    <form action="{{ route('admin.news.destroy', $article) }}" 
                                        ...
                                        <button type="button" 
                                                ...
                                                data-title="{{ $article->title }}"
                                                data-image="{{ $article->image ? 'oui' : 'non' }}"
                                                title="Supprimer">
            ...
            <div class="news-footer">
                <div class="news-stats-info">
                    <div class="stats-item">
                        <i class="fas fa-newspaper"></i>
                        <span><span class="stats-value">{{ $articles->count() }}</span> article{{ $articles->count() > 1 ? 's' : '' }}</span>
                    </div>
                    <div class="stats-item">
                        <i class="fas fa-eye"></i>
                        <span><span class="stats-value">{{ $articles->where('is_active', true)->count() }}</span> actif{{ $articles->where('is_active', true)->count() > 1 ? 's' : '' }}</span>
                    </div>
                    <div class="stats-item">
                        <i class="fas fa-calendar-alt"></i>
                        <span><span class="stats-value">{{ $articles->where('is_event', true)->count() }}</span> événement{{ $articles->where('is_event', true)->count() > 1 ? 's' : '' }}</span>
                    </div>
                </div>
                <a href="{{ route('admin.news.create') }}" class="btn-add-article">
                    <i class="fas fa-plus"></i>
