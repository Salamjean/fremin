...
<div class="prog-form-container">
    <div class="form-card">
        <div class="form-header">
            <h1 class="form-title">{{ isset($opportunity) ? 'Modifier l\'Appel' : 'Nouvel Appel en Cours' }}</h1>
        </div>
        ...
        <form
            ...
            <div class="form-section">
                <h3 class="section-title"><i class="fas fa-image"></i> Visuel de l'Appel</h3>
                <div class="image-preview" id="previewContainer">
                    @if(isset($opportunity) && $opportunity->image)
                        <img src="{{ asset('storage/' . $opportunity->image) }}" id="preview">
                    @else
            ...
            <div class="form-section">
                <h3 class="section-title"><i class="fas fa-bullhorn"></i> Informations Principales</h3>
                <div class="form-group">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label class="form-label">Type d'opportunité</label>
                            <select name="type" class="form-control" required>
                                <option value="Appel à projets" {{ old('type', $opportunity->type ?? '') == 'Appel à projets' ? 'selected' : '' }}>Appel à projets</option>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control" required>
                                <option value="open" {{ old('status', $opportunity->status ?? 'open') == 'open' ? 'selected' : '' }}>Ouvert</option>
                                <option value="closed" {{ old('status', $opportunity->status ?? '') == 'closed' ? 'selected' : '' }}>Clôturé</option>
                                <option value="upcoming" {{ old('status', $opportunity->status ?? '') == 'upcoming' ? 'selected' : '' }}>À venir</option>
                            </select>
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label">Description de l'appel</label>
            ...
            <div class="form-section">
                <h3 class="section-title"><i class="far fa-calendar-alt"></i> Dates & Localisation</h3>
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label class="form-label">Date limite (Deadline)</label>
                            <input type="date" name="deadline" class="form-control"
                                value="{{ old('deadline', isset($opportunity) && $opportunity->deadline ? $opportunity->deadline->format('Y-m-d') : '') }}">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
            ...
            <div class="form-section">
                <h3 class="section-title"><i class="fas fa-external-link-alt"></i> Actions</h3>
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label class="form-label">Lien de candidature</label>
                            <input type="url" name="link" class="form-control"
                                value="{{ old('link', $opportunity->link ?? '') }}">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="link_text" class="form-control"
                                value="{{ old('link_text', $opportunity->link_text ?? 'Postuler') }}">
                        </div>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <input type="number" name="sort_order" class="form-control"
                        value="{{ old('sort_order', $opportunity->sort_order ?? 0) }}" required>
                </div>
                <div class="form-group">
                    <label class="d-flex align-items-center gap-2">
                        <input type="checkbox" name="is_active" {{ old('is_active', $opportunity->is_active ?? true) ? 'checked' : '' }}>
                        Publier cet appel immédiatement
                    </label>
                </div>
            </div>

            <button type="submit" class="btn-submit">
                {{ isset($opportunity) ? 'Mettre à jour' : 'Publier l\'Appel' }}
            </button>
        </form>
    </div>
</div>
